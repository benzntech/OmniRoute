...
test("Qwen guide-settings POST creates valid DB-backed key (no keyId)", async () => {
  ...
  // Verify .env file was written with a valid DB-backed key
  const envPath = path.join(DUMMY_HOME, ".qwen", ".env");
  const envContent = await fs.readFile(envPath, "utf-8");

  // Extract OPENAI_API_KEY from .env
  const keyMatch = envContent.match(/^OPENAI_API_KEY=(.+)$/m);
  assert.ok(keyMatch, ".env should contain OPENAI_API_KEY");

  const apiKey = keyMatch[1];
  assert.notEqual(apiKey, "sk_omniroute", ".env should not contain placeholder");
  assert.ok(apiKey.startsWith("sk-"), "Key should start with sk- prefix");

  // Verify the key is valid in DB
  const valid = await validateApiKey(apiKey);
  assert.equal(valid, true, "Key in .env should validate in DB");
});
...
test("Qwen guide-settings POST with keyId uses existing key", async () => {
  ...
  // Verify .env uses the existing key
  const envPath = path.join(DUMMY_HOME, ".qwen", ".env");
  const envContent = await fs.readFile(envPath, "utf-8");

  const keyMatch = envContent.match(/^OPENAI_API_KEY=(.+)$/m);
  assert.ok(keyMatch);
  assert.equal(keyMatch[1], existingKey, "Should use existing key when keyId provided");
});
